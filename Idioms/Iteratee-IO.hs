-- Iteratee I/O
-- ref: https://wiki.haskell.org/Iteratee_I/O
-- ref: https://wiki.haskell.org/Iteratee
-- oleg: http://okmij.org/ftp/Haskell/Iteratee/

-- Avoid problem with lazy I/O
...
-- iteratee/iterIO/enumerator/pipes/conduit/liboleg/io-stream

-- Haskell98!

-- Pure Iteratees and parsing combinators
...
-- Chunk str gives the currently available part of the stream:
...
data Stream = EOF (Maybe ErrMsg) | Chunk String deriving Show
...
-- The basic design suggests several implementations. Here is the latest:
...
-- the State monad over stream: the step function consumes
-- (part of) the stream, computes the new state and returns
-- the unconsumed part of the stream.
-- The first argument to IE_cont, if non-empty, is the error indicator,
-- or a control message in general.
-- Since we carry the continuation anyway, the exception/error
-- is generally recoverable: we get resumable exceptions by default.
-- Indeed, the EOF error is essentially a control message, a request
-- for more data. If more data do become available, the error can be recovered
-- from.

-- It turns out, Iteratee forms a monad. That should not be too surprising
-- given that the step function does look like the State monad, and
-- IE_cont is like the `operating system call' to read in another chunk.
-- So, Iteratee is a State + Cont monad (with a back-trackable state).
-- We can use the familiar `do' notation for composing Iteratees.
...
-- It is easy to see that the monad laws are satisfied
...
had a drawback: a Done Iteratee returned by an Enumerator
could contain the ``rest of the stream''. This is meaningless
since a stream is produced by the enumerator and is the resource
of the enumerator, disposed at the end of the traversal. We should
not leak any of enumerator's state and should return
Chunk [] as the second argument of the IE_done constructor.
However, we could not enforce that requirement.
Another drawback was a more complex Monad instance, shown below.

-- The second argument to IE_done is the remaining part of the stream;
-- it should really be the remaining part of the chunk that the Iteratee
-- has received earlier. We can enforce this linearity restriction using
-- the rank-2 types. If the stream chunk is a string or any array for that
-- matter, we can write just IE_done a String; the empty array indicates
-- the absence of the remainder. 

instance Monad Iteratee where
    return x = IE_done x (Chunk "")

    IE_done x (Chunk "") >>= f = f x
    IE_done x stream     >>= f = case f x of
...
-- Justification for the case IE_done x s >>= f:
-- if (f x) is IE_done x s', then s' must be (Chunk "") because an
-- iteratee must always return the remainder of the received input.
-- Since (f x) returned IE_done without reading any input, the rest
-- of the stream s remains unconsumed.
-- If the linearity of the stream is enforced statically,
-- this return value of f x is likewise enforced.
-- If f x returns a control message, the remainder of the input s
-- is disregarded: the input buffer is flushed before the control.
-}

-- Throw an irrecoverable error
throwErr :: ErrMsg -> Iteratee a
throwErr e = IE_cont (Just e) (\s -> (throwErr e, s))

-- Throw a recoverable error
throwRecoverableErr :: ErrMsg -> (Stream -> (Iteratee a,Stream)) -> Iteratee a
throwRecoverableErr e k = IE_cont (Just e) k


-- Send EOF to Iteratee and disregard the unconsumed part of the stream
run :: Iteratee a -> a
run (IE_done x) = x
run (IE_cont Nothing k) = check . fst $ k (EOF Nothing)
 where
 check (IE_done x)   = x
 check (IE_cont e _) = error $ "possibly divergent, control message: " ++ show e
run (IE_cont (Just e) _) = error $ "control message: " ++ show e

{- Here is the third implementation

newtype Iteratee a = Iteratee{runIter:: Stream -> IterV a}
data IterV a = IE_done a Stream | IE_cont (Iteratee a) (Maybe ErrMsg)

instance Monad Iteratee where
    return x = Iteratee (IE_done x)
    Iteratee m >>= f = Iteratee $ 
	                 \s -> case m s of
				 IE_done x s -> runIter (f x) s
				 IE_cont m e -> IE_cont (m >>= f) e


-- See IterateeM.hs for the discussion of its drawbacks.

-- Throw an unrecoverable error
throwErr :: ErrMsg -> Iteratee a
throwErr e = Iteratee (\s -> IE_cont (throwErr e) (Just e))

-- Throw a recoverable error
throwRecoverableErr :: ErrMsg -> Iteratee a -> Iteratee a
throwRecoverableErr e i = Iteratee (\_ -> IE_cont i (Just e))
...
{-
tryIter :: Iteratee a -> Iteratee (Either ErrMsg a)
tryIter (Iteratee m) = Iteratee $
  \s -> case m s of
	  IE_done x s -> IE_done (Right x) s
	  IE_cont m Nothing -> IE_cont (tryIter m) Nothing
	  IE_cont _ (Just e)    -> IE_cont (Iteratee (IE_done (Left e))) Nothing

-- The last line could have been like the one below:
--	  IE_cont _ (Just e)    -> IE_done (Left e) s
-- That amounts to back-tracking on the stream. By default, we avoid any
-- backtracking, as a possible memory leak. Also, the stream in question
-- may be a linear stream and backtracking is not possible.
-- If one wishes to use a back-tracking parser, one has to transform
-- the stream into the one that permits backtracking and has the backing
-- storage for that.
-}
...
ie_cont :: (Stream -> (Iteratee a, Stream)) -> Iteratee a
ie_cont = IE_cont Nothing

ie_contM :: (Stream -> (Iteratee a, Stream)) -> (Iteratee a, Stream)
ie_contM k = (IE_cont Nothing k, empty_stream)

-- The analogue of List.break
-- It takes a character predicate and returns a string of characters,
-- which is the (possibly empty) prefix of the stream. None of the
-- characters in the string satisfy the character predicate.
-- If the stream is not terminated, the first character of the remaining
...
head = ie_cont step
 where
 step (Chunk "")    = (head, empty_stream)
 step (Chunk (c:t)) = (IE_done c, (Chunk t))
 step stream        = (IE_cont (Just (setEOF stream)) step, stream)
...
heads str = loop 0 str
 ...
 step cnt str (Chunk "")          = (loop cnt str, empty_stream)
...
skip_till_eof :: Iteratee ()
skip_till_eof = ie_cont step
 where
 step (Chunk _) = (skip_till_eof, empty_stream)
 step s         = (IE_done (), s)
...
drop n = ie_cont step
 where
 step (Chunk str) | length str <= n = (drop (n - length str), empty_stream)
 step (Chunk str) = (IE_done (), (Chunk (Prelude.drop n str)))
 step stream      = (IE_done (), stream)

-- Check if the stream is finished
is_finished :: Iteratee (Maybe ErrMsg)
is_finished = ie_cont check
 where check (Chunk "") = (is_finished, empty_stream)
       check s@(EOF e)  = (IE_done (Just $ setEOF s), s)
       check s          = (IE_done Nothing, s)


-- ------------------------------------------------------------------------
-- Combining the primitive iteratees to solve the running problem:
-- reading the headers and the content from an HTTP-like stream
...
read_lines = lines' []
 ...
 check acc _  0  = return . Left  . reverse $ acc -- no terminator was found
...
-- In the present file, Iteratees don't have any effects. Enumerators may:
-- they may need to read from a file. Hence we need the following type.
-- In the main library IterateeM.hs, both Iteratees and Enumerators
-- could be effectful, and we don't need two types of Enumerators
...
-- A `good' iteratee must move to the done state upon receiving the EOF
-- It is almost the same as 'run' -- modulo error handling
-- The two enumerators show a pattern that will occur again and again
-- (see the GUIDELINES in IterateeM.hs)
enum_eof :: Enumerator a
enum_eof (IE_cont Nothing k) = check . fst $ k (EOF Nothing)
 where
 check i@IE_done{} = i
 check i@(IE_cont (Just _) _) = i
 check _ = throwErr exc_divergent
enum_eof i = i

-- Another primitive enumerator: tell the Iteratee the stream terminated
-- with an error
enum_err :: ErrMsg -> Enumerator a
enum_err e (IE_cont Nothing k) = check . fst $ k (EOF (Just e))
 where
 check i@IE_done{} = i
 check i@(IE_cont (Just _) _) = i
 check _ = throwErr exc_divergent
...
enum_pure_1chunk str (IE_cont Nothing k) = fst $ k (Chunk str)
...
enum_fd fd iter = allocaBytes (fromIntegral buffer_size) (loop iter)
 where
  ...
  do_read k p = do
   ...
   case n of
    Left errno -> return . fst $ 
                   k (EOF (Just (toException (ErrorCall "IO error"))))
...
--
-- Stream adapters, or Enumeratees, handle nested (or, encapsulated) streams. 
-- Stream nesting is rather common: buffering, character encoding, compression,
-- encryption, SSL are all examples of stream nesting. On one
...
-- in an Iteratee for the outer stream, taking data from the parent
-- enumerator.
...
--
-- That many-to-many correspondence between the outer and nested streams
-- brings a complication. Suppose that the enumeratee has received EOF on
-- its, that is, the outer stream. The enumeratee, as the outer iteratee,
-- must move to the Done state.  Yet the nested iteratee is not finished.
-- The enumeratee then has to return the nested iteratee as its result.

type Enumeratee a = Iteratee a -> Iteratee (Iteratee a)


-- One of the simplest Enumeratee: the nested stream is a prefix
-- of the outer stream exactly n elements long. Such nesting arises
...
--
-- which should not surprise us since we already have
...
take n (IE_cont Nothing k)  = ie_cont (step n k)
 ...
 step n k chunk@(Chunk str) | length str < n = 
            (take (n - length str) . fst $ k chunk, empty_stream)
 step n k (Chunk str) = (IE_done (fst $ k (Chunk s1)), (Chunk s2))
  where (s1,s2) = splitAt n str
...
-- What do we do? Earlier, we chose to disregard the problem.
...
-- the recovery will be to report the accumulated nested iteratee.
...
enum_chunk_decoded iter = read_size
 ...
 check_size str@(_:_) =
     ...
     read_hex 0 str
 check_size _ = frame_err (exc "Error reading chink size") iter
...
read_lines_rest :: Iteratee (Either [Line] [Line], String)
read_lines_rest = do
		  ls   <- read_lines
		  rest <- break (const False)
		  return (ls,rest)
testp1 =
    let (Right lines, rest)
	    = run $ enum_pure_1chunk test_str1 read_lines_rest
    in
...
testp2 =
    let (Right lines, rest)
	    = run $ enum_pure_nchunk test_str1 5 read_lines_rest
    in
    lines == ["header1: v1","header2: v2","header3: v3","header4: v4",
	      "header5: v5","header6: v6","header7: v7"]
    && rest == "rest\n"
...
test_driver filepath = do
  ...
  result <- fmap run $ enum_fd fd read_lines_and_one_more_line
  closeFd fd
...
test11 = test_driver "test1.txt" -- Complete headers, up to "header7: v7"
test12 = test_driver "test2.txt" -- The same
test13 = test_driver "test3.txt" -- "header3: v3", then EOF
test14 = test_driver "/dev/null" -- Incomplete headers [], EOF
...
test_driver_full filepath = do
 ...
 where
  read_headers_body = do
     headers <- read_lines
     body    <- return . run =<< enum_chunk_decoded read_lines
     status  <- is_finished
     return (headers,body,status)

test21 = test_driver_full "test_full1.txt"
{-
Complete headers
...
test22 = test_driver_full "test_full2.txt"
-- *** Exception: control message: Just "Frame error"


test23 = test_driver_full "test_full3.txt"
...
--  LocalWords:  Enumeratee enumeratee
...
instance Monad m => Monad (Iteratee el m) where
    ...
    IE_cont e k >>= f = IE_cont e (\s -> k s >>= docase)
     where
     docase (IE_done a, s)   = case f a of
		   IE_cont Nothing k -> k s
		   i                 -> return (i,s)
...
instance Functor m => Functor (Iteratee el m) where
  {-# INLINE fmap #-}
  fmap f (IE_done x)   = IE_done (f x)
  fmap f (IE_cont e k) = IE_cont e (fmap (\ (i,s) -> (fmap f i,s)) . k)

instance (Functor m, Monad m) => Applicative (Iteratee el m) where
    {-# INLINE pure #-}
    pure = IE_done

    {-# INLINE (<*>) #-}
    IE_done f   <*> m = fmap f m
    IE_cont e k <*> IE_done x = IE_cont e (fmap docase . k)
     where
     docase (IE_done f, s)   = (IE_done (f x), s)
     docase (IE_cont e k, s) = (IE_cont e (fmap docase . k), s)
    IE_cont e k <*> x@(IE_cont ex kx) = IE_cont e (\s -> k s >>= docase)
     where
     docase (IE_done f,s@(EOF Nothing)) = return (fmap f x, s)
     docase (IE_done f, s) = case ex of
       -- Here we really do not want to return (fmap f x, s)
       -- where we know that the iteratee (fmap f x) can consume
       -- the stream s. 
       Nothing -> fmap (\ (i,s) -> (fmap f i,s)) (kx s)
       ex      -> return (fmap f x, s)
     docase (IE_cont e k, s)    = return (IE_cont e (\s -> k s >>= docase), s)
